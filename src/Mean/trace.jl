...
trace = MeanCompliance(problem, solver; kwargs..., method = :approx, sample_method = :hadamard, sample_once = true, nv = 10)
...
fname = "$out_dir/trace_hadamard10_mean_csimp"
if TopOpt.TopOptProblems.getdim(problem) == 2
    image = TopOptProblems.RectilinearTopology(problem, solver.penalty.proj.(filterT(Val(true), solver, rmin)(result.topology)))
...
@show trace(result.topology)
@show exact_svd(result.topology) / trace(result.topology)
